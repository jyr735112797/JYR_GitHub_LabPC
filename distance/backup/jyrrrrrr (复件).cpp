...
{
double interval = 0.5;
std::vector<geometry_msgs::Point> plannn;
      ...
      plannn.push_back(srv.response.plan.poses[i].pose.position);
      }

          //plan[1]= srv.response.plan.poses[1].pose.position.x;
         // plan[1].y = srv.response.plan.poses[1].pose.position.y;
        for(size_t i = 1; i < (plannn.size()); i++)
        {

        ROS_INFO("planx = %f, y = %f",plannn[i].x ,plannn[i].y);
        ...
        {
            geometry_msgs::Point last_pose = plannn[0];
            ...
            {
                geometry_msgs::Point pose = plannn[i];
                ...
                if(dis >= interval)
                {
                    ...
                    listener.plannnnn_.push_back(last_pose);
                    //w.push_back(last_wwww);
                    ROS_INFO("interval x=%f,y=%f",last_pose.x,last_pose.y);
                }
...
ROS_INFO("planx = %f, y = %f",listener.plannnnn_[i].x ,listener.plannnnn_[i].y);
...
while (ros::ok())
{
 ROS_INFO("count = %lfd    %lf",listener.plannnnn_[1].x,listener.plannnnn_[1].y);
  if((listener.count == 40)&&(i <(listener.plannnnn_.size()-1)))
 {
   ROS_INFO("dasd");
   goal.target_pose.header.frame_id = "map";
   goal.target_pose.header.stamp = ros::Time::now();
  //goal.target_pose.pose.position =MoveBase.plannnnn_[i];
  goal.target_pose.pose.position.x = listener.plannnnn_[i].x;
   goal.target_pose.pose.position.y = listener.plannnnn_[i].y;
  goal.target_pose.pose.orientation.w = 1.0;//listener.planwwwww_[i].w;//1.0;
