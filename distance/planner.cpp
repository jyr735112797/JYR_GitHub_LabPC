#include </home/jyr/catkin_ws/src/distance/include/planner.h>
#include <cmath>
...
int main(int argc, char** argv){
  ...
  //ros::NodeHandle nh2;
  //ros::Subscriber get_goal = nh2.subscribe<geometry_msgs::PoseStamped>("goal", 1, &Listener::getrvizgoal,&listener);
...
//planner_costmap_ros_->start();
   ...
   // =new global_planner::GlobalPlanner("global_planner", planner_costmap_ros_, "map");
   // double origin_x = planner_costmap_ros_->getOriginX(), origin_y = planner_costmap_ros_->getOriginY();
    //make plan
    geometry_msgs::Point starttttt ;
    starttttt.x = x;//-0.37695;//x;//0.3
    starttttt.y = y;//-0.364562;//y;//1


    //ROS_INFO("x=%lf,y=%lf",origin_x,origin_y);
    geometry_msgs::Point goalllll ;
    goalllll.x=6.75;//2.85;
    goalllll.y = 4.0;//5.25;
    double interval=0.5;
    //std::vector<geometry_msgs::Point> plannnnn;
   // std::vector<geometry_msgs::PoseStamped> pppp;


    makeGlobalPlan(starttttt,goalllll,interval,plannnnn_,planwwwww_);

   // geometry_msgs::PoseStamped global_pose;
    //planner_costmap_ros_->start();
    //clear the planner's costmap
    //ROS_INFO("x=%f,y=%f",global_pose.pose.position.x,global_pose.pose.position.y);
 // nav
...
/*
    ...
    goal.target_pose.pose.position.x =temp_goal.pose.position.x;
    goal.target_pose.pose.position.y = temp_goal.pose.position.y;
    goal.target_pose.pose.orientation.w = temp_goal.pose.orientation.w;
/*
    /*
    while (ros::ok())
    {
     size_t i =1;
      ...
      if((listener.count == 40)&&i <(plannnnn_.size()-1))
     {

       goal.target_pose.header.frame_id = "map";
       goal.target_pose.header.stamp = ros::Time::now();
       goal.target_pose.pose.position =plannnnn_[i];
       //goal.target_pose.pose.position.y = plannnnn_[i].y;
        ROS_INFO("Sending goal x:%lf  y:%lf",goal.target_pose.pose.position.x,goal.target_pose.pose.position.y);
       ac.sendGoal(goal);
       ac.waitForResult();
       if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
     ...
     /*
    for(size_t i = 1;i <(plannnnn_.size()-1);i++)
      {
        goal.target_pose.header.frame_id = "map";
        goal.target_pose.header.stamp = ros::Time::now();
        goal.target_pose.pose.position =plannnnn_[i];
